    ...
    public function generate(\ReflectionClass $original, PhpClass $proxy)
    {
        ...
        $proxy->setMethod(PhpMethod::create('prepareArguments')
            ...
            ->setBody($writer->reset()->writeln('$processed = array();')
                 ...
                 ->indent()
                     ->writeln('if (($arg instanceof \Symfony\Component\DependencyInjection\Parameter) || (preg_match("/^\%(.*)\%$/", $arg, $matches))) {')
                     ->indent()
                         ->writeln('if (isset($matches[1])) {')
                     ->indent()
                             ->writeln('$arg = $matches[1];')
                         ->outdent()
                         ->writeln('}')
                         ->writeln('$processed[] = $this->container->getParameter((string) $arg);')
                     ->outdent()
                     ->writeln('} else if ($arg instanceof \Symfony\Component\DependencyInjection\Reference) {')
                     ->indent()
